...
export const alt = "Regional epidemic surveillance — HealthWatch Global";
...
const VALID = ["africa", "asia", "europe", "americas", "oceania"];

type Row = { active: boolean; cases: number | null; country_en: string | null; country: string | null; disease_en: string | null; disease: string | null; region: string | null };

export default async function RegionOgImage({
  params,
}: {
  ...
  const { data } = await supabase
    .from("outbreaks")
    .select("active, cases, country_en, country, disease_en, disease, region")
    .ilike("region", region);

  const rows       = data as Row[] ?? [];
  const activeCount  = rows.filter((o) => o.active).length;
  const totalCases   = rows.reduce((s, o) => s + (o.cases ?? 0), 0);
  const countryCount = new Set(rows.map((o) => o.country_en || o.country).filter(Boolean)).size;
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", padding: "7px 16px", borderRadius: 20, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <span style={{ fontSize: 12, color: "#9ca3af" }}>REGIONAL SURVEILLANCE</span>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: 1, justifyContent: "center", position: "relative", zIndex: 1, marginTop: 20 }}>
          ...
          <div style={{ display: "flex", fontSize: 20, color: "#6b7280", marginTop: 14 }}>
            Epidemic intelligence · WHO & ECDC & PAHO data
          </div>
        </div>
