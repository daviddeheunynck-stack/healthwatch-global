...
export async function generateMetadata({
  ...
  const DESC_TPL: Record<Locale, string> = {
    en: `${active.length} active outbreak${active.length !== 1 ? "s" : ""} in ${countryEn}. WHO, ECDC, PAHO and Africa CDC data tracked daily by HealthWatch Global.`,
    fr: `${active.length} foyer${active.length !== 1 ? "s" : ""} actif${active.length !== 1 ? "s" : ""} en ${countryEn}. Données OMS, ECDC, PAHO et Africa CDC suivies quotidiennement par HealthWatch Global.`,
    es: `${active.length} brote${active.length !== 1 ? "s" : ""} activo${active.length !== 1 ? "s" : ""} en ${countryEn}. Datos de OMS, ECDC, PAHO y Africa CDC seguidos diariamente.`,
    ar: `${active.length} تفشٍّ نشط في ${countryEn}. بيانات منظمة الصحة العالمية، ECDC، PAHO وAfrica CDC.`,
    id: `${active.length} wabah aktif di ${countryEn}. Data WHO, ECDC, PAHO dan Africa CDC dipantau harian.`,
  };
