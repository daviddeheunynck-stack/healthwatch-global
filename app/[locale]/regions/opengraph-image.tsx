...
export const alt = "Disease outbreaks by WHO region — HealthWatch Global";
...
export default async function RegionsIndexOgImage() {
  ...
  const { data } = await supabase
    .from("outbreaks")
    .select("active, cases, region");

  const rows = (data ?? []) as { active: boolean; cases: number | null; region: string | null }[];

  const activeCount   = rows.filter((o) => o.active).length;
  const totalCases    = rows.reduce((s, o) => s + (o.cases ?? 0), 0);
  const activeRegions = new Set(
    rows.filter((o) => o.active && o.region)
      .map((o) => o.region!.toLowerCase().trim())
      .filter((r) => (REGION_SLUGS as readonly string[]).includes(r))
  ).size;
  const isActive = activeCount > 0;

  const activeStr  = activeCount > 0 ? String(activeCount) : "—";
  const caseStr    = totalCases > 0 ? fmt(totalCases) : "—";
  const regionStr  = activeRegions > 0 ? String(activeRegions) : "—";
  const activeLabel = activeCount === 1 ? "Active outbreak" : "Active outbreaks";
  const regionLabel = activeRegions === 1 ? "Region affected" : "Regions affected";
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", padding: "7px 16px", borderRadius: 20, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <span style={{ fontSize: 12, color: "#9ca3af" }}>5 WHO REGIONS</span>
          </div>
        </div>
        ...
        <div style={{ display: "flex", position: "relative", zIndex: 1, marginTop: 40, border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, overflow: "hidden", background: "rgba(255,255,255,0.025)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "26px 0", flex: 1, borderRight: "1px solid rgba(255,255,255,0.06)", gap: 6 }}>
            <span style={{ fontSize: 46, fontWeight: 800, color: isActive ? "#f87171" : "#374151", letterSpacing: -1, lineHeight: 1 }}>{regionStr}</span>
            <span style={{ fontSize: 13, color: "#6b7280" }}>{regionLabel}</span>
          </div>
