 ...
 * ligne : tous les essais recevaient cet email.
...
// `unsubUrl` est obligatoire, pas optionnel : cet email n'avait AUCUN moyen de
// s'y soustraire jusqu'au 2026-08-23 — ni lien in-body, ni en-tete
// List-Unsubscribe — et son seul drapeau d'echappement (`no_weekly_signal`)
// n'etait ecrit que par des liens presents dans des emails qu'un utilisateur en
// essai n'a jamais recus au jour 8. Le rendre requis dans la signature garantit
// qu'on ne peut pas reintroduire ce cas par oubli.
...
async function runPilotFollowUp(_req: NextRequest, supabase: SupabaseClient) {
  ...
  const queryPilots = () =>
    createClient(SUPABASE_URL, SUPABASE_SERVICE)
      ...
      // fait — il ecrivait a TOUS les essais, un jour apres le J+7 de
      // onboarding-sequence (fenetres [-8.5j,-7.5j) contre [-7.5j,-6.5j)).
      // Pour revenir en arriere, retirer cette seule ligne — mais alors
      // renommer le cron, sans quoi la meme dette repart.
      .eq("is_pilot", true)
      .not("email", "is", null)
  ...
  // equivalents pour tout ce qui est marketing. Voir lib/mail-suppression.ts.
