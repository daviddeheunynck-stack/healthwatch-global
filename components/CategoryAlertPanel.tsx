            ...
            <div key={a.id} className="flex items-center justify-between gap-2 p-2.5 rounded-lg border border-gray-700/40 bg-gray-800/30">
              <div className="text-xs space-y-0.5 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <span className="text-red-400 font-semibold">≥ {a.min_cases.toLocaleString(locale === "ar" ? "ar-SA" : (locale || "en"))}</span>
                </div>
                <p className="text-[10px] text-gray-600">{a.email} · {c.lastFired}: {a.last_fired_at ? new Date(a.last_fired_at).toLocaleDateString(locale === "ar" ? "ar-SA" : locale) : c.never}</p>
              </div>
